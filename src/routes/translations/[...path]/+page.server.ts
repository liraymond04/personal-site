...
const run: PageServerLoad = async ({ params, parent }) => {
  ...
  let file_path = path.relative(repo_name, params_path);
  ...
  // re-add removed start_root
  const result = data.props.items.filter(item => params_path.includes(item.path))?.[0]
  const _path = `${dir}/${result.path}/index.md`

  const markdownContent = await files[_path]()
  const metadata = parseMetadata(markdownContent)

  if (metadata['start_root'] && !Array.isArray(metadata['start_root'])) {
    const start_root = metadata['start_root'];
    file_path = path.join(start_root, file_path)
  }
